...
export async function singleChatController(payload, ws) {
    try {
        // we will parse the data object contaning sender id group id and content
        console.log(payload);
        const { senderId, groupId, content, chatId } = WebSocketChatRequestPayloadSchema.parse(payload);
...
export async function groupChatController(payload) {
    try {
        // we will parse the data object contaning sender id group id and content
        const { senderId, groupId, content } = WebSocketChatRequestPayloadSchema.parse(payload);
        const savedChatRes = await Prisma.message.create({
            data: { senderId, groupId, content },
        });
        // check point --> if here then chat is saved on db now we need to push this chat to group members
        ...
        if (typeof groupUserIds != "undefined") {
            ...
            console.log("group map not properly .... how can a group id return undefined ");
            throw new Error("group map not working properply");
        }
    }
    catch (e) {
